...
class ARIMA_MODEL:
    ...
    def __init__(self, symbol: str, logaritm: bool = False):
        _data = FinanceData.getHistory(symbol, '1y')['High']
        _var = pandas.Series(_data.values)
        self.data = _var
        if logaritm:
            self.data = numpy.log(self.data)

        mask = (self.data.index < len(self.data) - 15)
